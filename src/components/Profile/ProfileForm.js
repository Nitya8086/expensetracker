...
const ProfileForm = () => {
  ...
  const getProfileData = useCallback(async () => {
    const res = await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyCrilnK_PD53JrvxxK3CSr_mN-b3fzjV40",
      {
        method: "POST",
        headers: {
    ...
    if (name && profile) {
      ...
      try {
        const response = await axios.post(
          "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyCrilnK_PD53JrvxxK3CSr_mN-b3fzjV40",
          body
        );
  ...
  const verifyEmailHandeler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key= AIzaSyCrilnK_PD53JrvxxK3CSr_mN-b3fzjV40",
        User
      );
